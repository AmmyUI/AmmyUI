    ...
    public class ExpressionBuilder
    {
        ...
        private Expression DeserializeNode(XElement el)
        {
            ...
            */
            ...
            if (cmd == "newvar") return DeserializeNewVar(el);
            if (cmd == "new") return DeserializeNew(el);
            if (cmd == "prim") return DeserializePrimitive(el);
            if (cmd == "assign") return DeserializeAssign(el);
            if (cmd == "prop") return DeserializeProperty(el);
            if (cmd == "field") return DeserializeField(el);
            if (cmd == "call") return DeserializeCall(el);
            if (cmd == "this") return DeserializeThis(el);
            if (cmd == "null") return DeserializeNull(el);
            if (cmd == "cast") return DeserializeCast(el);
            if (cmd == "staticcall") return DeserializeStaticCall(el);
            if (cmd == "staticfield") return DeserializeStaticField(el);
            if (cmd == "staticproperty") return DeserializeStaticProperty(el);
            if (cmd == "typeof") return DeserializeTypeof(el);
            if (cmd == "binary") return DeserializeBinary(el);
            if (cmd == "unary") return DeserializeUnary(el);
            if (cmd == "lambda") return DeserializeLambda(el);
            if (cmd == "parameter") return DeserializeParameter(el);
            if (cmd == "delegate") return DeserializeDelegate(el);
            if (cmd == "ternary") return DeserializeTernary(el);
            if (cmd == "arrayaccess") return DeserializeArrayAccess(el);
        ...
        private MethodInfo DeserializeMethodInfo(XElement el)
        {
            ...
            var isInstanceText = el.Attribute("isInstance").Value;
            var isInstance = isInstanceText.Equals("true", StringComparison.CurrentCultureIgnoreCase);

            if (elements[0].Name == "typeinfo") {
                var ownerType = DeserializeTypeInfo(elements[0]);
                return ownerType.GetMethod(methodName, isInstance);
            }

            var ownerNode = DeserializeNode(elements[0]);
            return ownerNode.Type.GetMethod(methodName, isInstance);
        }
...
#else
            return Expression.Constant(methodInfo.CreateDelegate(delegateType, methodInfo));
#endif
        }
        ...
        private Expression DeserializeEventAdd(XElement el)
        {
            ...
            var createDelegateMethod = typeof(MethodInfo).GetMethod("CreateDelegate", true, new[] { typeof(Type), typeof(object) });
            var eventHandler = Expression.Constant(_instance.GetType().GetMethod(eventHandlerName, true));
        ...
        private Expression DeserializeBinary(XElement el)
        {
            ...
            if (op == "Sum") {
                if (leftRight[0].Type == typeof(string) && leftRight[1].Type == typeof(string)) {
                    var concat = typeof(string).GetMethod("Concat", false, new[] { typeof(string), typeof(string) });
                    return Expression.Call(concat, leftRight[0], leftRight[1]);
                }

                if (leftRight[0].Type == typeof (string) || leftRight[1].Type == typeof (string)) {
                    var concat = typeof(string).GetMethod("Concat", false, new[] { typeof(object), typeof(object) });
        ...
        private Expression DeserializeTypeof(XElement el)
        {
            ...
            var typeofMethod = helpersType.GetMethod("FindType", false, new Type[0])
        ...
        private Expression DeserializeCall(XElement el)
        {
            ...
            */
            ...
            var method = instance.Type.GetMethod(methodName, true, parameters.Select(p => p.Type).ToArray());
        ...
        private Expression DeserializeStaticCall(XElement el)
        {
            ...
            var method = type.GetMethod(methodName, false, parameters.Select(p => p.Type).ToArray());
            var methodParameterTypes = method.GetParameters().Select(pi => pi.ParameterType).ToArray();
            var convertedParameters = parameters.Select((p, i) => methodParameterTypes[i] == p.Type ? p : Expression.Convert(p, methodParameterTypes[i])).ToArray();
        ...
        private Expression DeserializePrimitive(XElement el)
        {
            ...
            if (type.GetTypeInfo().IsEnum)
                return Expression.Constant(Enum.Parse(type, val.Substring(val.LastIndexOf(".", StringComparison.CurrentCultureIgnoreCase) + 1), true));
